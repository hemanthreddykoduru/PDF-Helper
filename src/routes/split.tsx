...
function SplitTool() {
  ...
  const [result, setResult] = useState<{ bytes: Uint8Array | Blob; name: string } | null>(null);
  ...
  const applyRange = () => {
    ...
    rangeInput.split(",").forEach((part) => {
      ...
      if (!isNaN(a) && !isNaN(b))
        for (let i = Math.max(1, a); i <= Math.min(thumbs.length, b); i++) next.add(i);
  ...
  const downloadSelected = async () => {
    ...
    try {
      ...
      const bytes = await extractPages(fileData.bytes as any, indices);
      const name = `${stripExtension(fileData.file.name)}-extract.pdf`;
      setResult({ bytes, name });
      await addRecent({ name, size: bytes.byteLength, operation: "Split — extract" }, bytes);
      toast.success("Extraction complete: Fragment recovered.");
    } catch (err) {
      toast.error("Extraction failed: Stream alignment lost.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const downloadPerPage = async () => {
    ...
    try {
      ...
      const pages = await splitToIndividualPages(fileData.bytes as any);
      const zip = new JSZip();
      pages.forEach((p) => zip.file(p.name, p.bytes));
      const blob = await zip.generateAsync({ type: "blob" });
      const name = `${stripExtension(fileData.file.name)}-pages.zip`;
      setResult({ bytes: blob, name });
      // Recent entry for ZIP (no bytes stored for ZIP history download for now to avoid bloat)
      await addRecent({ name, size: blob.size, operation: "Split — per page" });
      toast.success("Fragmentation complete: ZIP generated.");
    } catch (err) {
      toast.error("Split failed: ZIP synthesis error.");
    } finally {
      setBusy(false);
    }
  };
    ...
    <ToolPage>
      <div className="mx-auto max-w-4xl px-5 pt-4 pb-32">
        <div className="flex items-center gap-4 mb-8">
           ...
           <h1 className="text-xl font-black tracking-tight uppercase">Split PDF</h1>
        </div>
        ...
        {!result ? (
            ...
            {fileData && (
              <div className="mt-8 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="flex flex-col gap-4 rounded-[28px] border border-border/50 bg-surface p-6 sm:flex-row sm:items-center">
                   ...
                   <Button variant="ghost" size="sm" onClick={() => { setFileData(null); setSelected(new Set()); }}>
                     Change File
                   </Button>
                </div>
                        ...
                        {thumbs.map((t) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <div
                                className={cn(
                                  "absolute right-2 top-2 flex h-6 w-6 items-center justify-center rounded-full text-[9px] font-black shadow-sm transition-all animate-in zoom-in",
                                  isSel
                                    ? "bg-primary text-white scale-110"
                                    : "bg-black/40 text-white group-hover:bg-primary/80 group-hover:scale-105",
                                )}
                              >
                                {isSel ? <Check className="h-3 w-3" /> : t.page}
                              </div>
                              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-2 text-center">
                                <span className="text-[8px] font-black tracking-wider text-white uppercase font-sans">P{t.page}</span>
                              </div>
                            </button>
                          );
                        })}
                  ...
                  <div className="space-y-6">
                     <div className="rounded-[28px] border border-border/50 bg-surface p-6 space-y-4">
                        ...
                        <div className="space-y-3">
                          ...
                          />
                          <Button variant="secondary" className="w-full h-11" size="sm" onClick={applyRange}>
                            Inject Range
                          </Button>
                        </div>
                     </div>
          ...
          <ResultScreen 
            result={result} 
            onReset={reset} 
            operationLabel={result.name.endsWith(".zip") ? "File Bundle" : "Page Fragment"}
            successMessage={result.name.endsWith(".zip") ? "Document Fragmented to ZIP" : "Pages Extracted Successfully"}
          />
        )}
      </div>
      ...
      {busy && (
        <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-background/90 backdrop-blur-2xl animate-in fade-in duration-500">
          <div className="relative mb-8">
             ...
             <Scissors className="absolute inset-0 m-auto h-10 w-10 text-primary animate-pulse" />
          </div>
          <h2 className="text-2xl font-black tracking-tighter uppercase text-center px-6">Compiling Page Streams</h2>
